...
contract FlashReceiver is IFlashReceiver {
    ...
    function flashCallback(address[] calldata _pools, uint[] calldata _amts, uint[] calldata _fees, bytes calldata)
        public
    {
        for (uint i = 0; i < _pools.length; i++) {
            address token = ILendingPool(_pools[i]).underlyingToken();
            IERC20(token).transfer(_pools[i], _amts[i] + _fees[i]);
        }
    }
...
contract FlashReceiverMint is IFlashReceiver {
    ...
    function flashCallback(address[] calldata _pools, uint[] calldata _amts, uint[] calldata, bytes calldata) public {
...
contract FlashReceiverNoRepay is IFlashReceiver {
    ...
    function flashCallback(address[] calldata, uint[] calldata, uint[] calldata, bytes calldata) public {}
...
contract TestFlash is DeployAll {
    ...
    function testFlashDuplicatePools() public {
        ...
        bytes memory data;
        pools[0] = address(lendingPools[WETH]);
        pools[1] = address(lendingPools[USDT]);
        pools[2] = address(lendingPools[WETH]);
        vm.expectRevert('INC#306');
    ...
    function testFlash() public {
        ...
        amts[0] = 1e18;
        amts[1] = 1e10;
        cashes[0] = IERC20(WETH).balanceOf(pools[0]);
        cashes[1] = IERC20(USDT).balanceOf(pools[1]);
        deal(WETH, address(flashReceiver), amts[0] * 2);
        deal(USDT, address(flashReceiver), amts[1] * 2);
        flashReceiver.flash(pools, amts, data);
        assertGe(IERC20(WETH).balanceOf(pools[0]), cashes[0]);
        assertGe(IERC20(USDT).balanceOf(pools[1]), cashes[1]);
    }
}
