...
// import {PoolConfiguration, ModeConfiguration} from '@init-cap-interfaces/core/IConfig.sol';
// import {TokenFactors, IConfig, ModeStatus} from '../contracts/interfaces/core/IConfig.sol';
...
// import plugins
// import {MockSwapStrat} from '../contracts/mock/MockSwapStrat.sol';
...
contract DeployBase is Configurations {
    ...
    function _deployPlugins() internal {
        ...
        // mockSwapStrat = new MockSwapStrat(address(initCore), WMNT, SWAP_ROUTER_V2);
        musdusdyWrapHelper = new mUSDUSDYHelper(USDY, mUSD);
        MoneyMarketHook moneyMarketHookImpl = new MoneyMarketHook(address(initCore), WMNT);
    ...
    function _setConfigs() internal {
        ...
        for (uint i; i < modeConfigs.length; ++i) {
            ...
            for (uint j; j < modeConfig.tokens.length; j++) {
                ...
                lendingPoolList[j] = address(lendingPools[modeConfig.tokens[j]]);
            }
